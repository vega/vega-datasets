            ...
            try:
                ...
                with zipfile.ZipFile(zip_data) as zf:
                    ...
                    for tif_file in tif_files:
                        tif_path = temp_dir / os.path.basename(tif_file)
                        with open(tif_path, 'wb') as f:
                            f.write(zf.read(tif_file))
                        downloaded_files.append(tif_path)

                # Log connection info for debugging/optimization
                logger.debug(f"Connection info for {url}: "
                           f"Established in {response.conn_info.established_latency}, "
                           f"Headers: {response.headers}")  # Changed line

            except niquests.RequestException as e:
                logger.error(f"Error downloading from {url}: {str(e)}")
                continue
            except zipfile.BadZipFile as e:
                logger.error(f"Error extracting zip from {url}: {str(e)}")
                continue
...
def process_habitat_data(vector_filepath: Path, raster_filepaths: Sequence[Path]) -> pd.DataFrame:
    ...
    for raster in raster_filepaths:
        stem = os.path.basename(raster).split('.')[0]
...
def main() -> None:
    ...
    if not VECTOR_FP.exists():
        logger.error(f"Vector file not found at {VECTOR_FP}")
        return
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            logger.info(f"Data successfully exported to {OUTPUT_CSV_NAME}")
