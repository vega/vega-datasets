...
`gallery-examples.json` is generated by this repo, but the shape of its records is defined as a standalone [Table Schema](https://datapackage.org/standard/table-schema/) at [`_data/gallery-examples-schema.json`](_data/gallery-examples-schema.json). Any collection of Vega / Vega-Lite / Altair examples — a tutorial site, a course gallery, a domain-specific notebook set — can publish its own index conforming to the schema: seven fields (`gallery_name`, `example_name`, `example_url`, `spec_url`, `categories`, `description`, `datasets`) with `spec_url` as the primary key. Downstream consumers (LLMs, search tools, example aggregators) can then federate across every published index without per-source integration code.
