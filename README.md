...
`gallery-examples.json` is generated by this repo, but its record shape is defined as a [Table Schema](https://datapackage.org/standard/table-schema/) inside [`datapackage.json`](datapackage.json) under the `gallery_examples` resource. Any collection of Vega / Vega-Lite / Altair examples — a tutorial site, a course gallery, a domain-specific notebook set — can publish its own index conforming to the schema: seven fields (`gallery_name`, `example_name`, `example_url`, `spec_url`, `categories`, `description`, `datasets`) with `spec_url` as the primary key. `fieldsMatch: "subset"` lets publishers add extension columns without breaking conformance, and downstream consumers (LLMs, search tools, example aggregators) can federate across every published index without per-source integration code.

To validate your own index against the contract:

```bash
jq '.resources[] | select(.name == "gallery_examples") | .schema' datapackage.json > schema.json
frictionless validate your-index.json --schema schema.json
```

Pin to a specific contract revision by citing a release tag in the URL: `https://raw.githubusercontent.com/vega/vega-datasets/v3.2.1/datapackage.json`. The schema is versioned as part of the vega-datasets package ([SemVer](https://datapackage.org/recipes/data-package-version/) — MAJOR on breaking schema changes).
